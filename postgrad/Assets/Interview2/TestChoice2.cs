...
public class TestChoice2 : MonoBehaviour
{
    [SerializeField]
    ...
    private void Update()
    {
        //if (!flag) 
        //{
        if (Input.GetKeyDown(KeyCode.C))   //c키 누를시   대화 생성   //단 한번만 일어나도록 해야함.
        {
            StartCoroutine(ACoroutine());
            //if (state == true)
            //{
    ...
    IEnumerator ACoroutine()    //나중에 public Dialague dialague1; 별로 만들어서 리턴값을 따로 변수에 저장시켜 조건 나누기
    {
        ...
        if (theChoice.GetResult() == 0)
        {
            ...
            Debug.Log("심교수님 : " + PlayerPrefs.GetInt("shim"));    //결과 확인하기
            Destroy(gameObject);
            SceneManager.LoadScene("OT");
        }
        if (theChoice.GetResult() == 1)
        {
            PlayerPrefs.SetInt("lee", PlayerPrefs.GetInt("lee") + 10);
            Debug.Log("이교수님 : " + PlayerPrefs.GetInt("lee"));    //결과 확인하기
            Destroy(gameObject);
            SceneManager.LoadScene("OT");
        }
        if (theChoice.GetResult() == 2)
        {
            PlayerPrefs.SetInt("lee", PlayerPrefs.GetInt("lee") + 5);
            PlayerPrefs.SetInt("shim", PlayerPrefs.GetInt("shim") + 1);
            PlayerPrefs.SetInt("kim", PlayerPrefs.GetInt("kim") + 3);
            Debug.Log("이교수님 : " + PlayerPrefs.GetInt("lee"));    //결과 확인하기
            Debug.Log("심교수님 : " + PlayerPrefs.GetInt("shim"));    //결과 확인하기
