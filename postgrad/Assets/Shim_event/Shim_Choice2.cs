...
public class Shim_Choice2 : MonoBehaviour
{
    [SerializeField]
    ...
    void Start()
    {
        ...
        test = GameObject.Find("TestChoice2");
        state = true;
    }
    ...
    private void Update()
    {
        //if (!flag) 
        //{
        if (Input.GetKeyDown(KeyCode.F))   //X키 누를시   대화 생성   //단 한번만 일어나도록 해야함.
        {
            StartCoroutine(ACoroutine());
            //if (state == true)
            //{
    ...
    IEnumerator ACoroutine()    //나중에 public Dialague dialague1; 별로 만들어서 리턴값을 따로 변수에 저장시켜 조건 나누기
    {
        ...
        if (theChoice.GetResult() == 0) // 비틀즈
        {
            PlayerPrefs.SetInt("shim", PlayerPrefs.GetInt("shim") - 10);
            Debug.Log("심교수님 이벤트2 : " + PlayerPrefs.GetInt("shim"));    //결과 확인하기
            SceneManager.LoadScene("Kim_Event");
        }
        if (theChoice.GetResult() == 1)  // 어플
        {
            PlayerPrefs.SetInt("shim", PlayerPrefs.GetInt("shim") + 10);
            Debug.Log("심교수님 이벤트2 : " + PlayerPrefs.GetInt("shim"));    //결과 확인하기
            SceneManager.LoadScene("Kim_Event");
