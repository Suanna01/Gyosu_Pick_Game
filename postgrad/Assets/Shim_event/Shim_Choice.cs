...
public class Shim_Choice : MonoBehaviour
{
    [SerializeField]
    ...
    private void Update()
    {
        //if (!flag) 
        //{
        if (Input.GetKeyDown(KeyCode.D))   //X키 누를시   대화 생성   //단 한번만 일어나도록 해야함.
        {
            StartCoroutine(ACoroutine());
            //if (state == true)
            //{
    ...
    IEnumerator ACoroutine()    //나중에 public Dialague dialague1; 별로 만들어서 리턴값을 따로 변수에 저장시켜 조건 나누기
    {
        ...
        if (theChoice.GetResult() == 0) // 지하철
        {
            PlayerPrefs.SetInt("shim", PlayerPrefs.GetInt("shim") - 1);
            Debug.Log("심교수님 이벤트1 : " + PlayerPrefs.GetInt("shim"));    //결과 확인하기
            SceneManager.LoadScene("Shim_Event2");
        }
        if (theChoice.GetResult() == 1)  // 버스
        {
            PlayerPrefs.SetInt("shim", PlayerPrefs.GetInt("shim") - 1);
            Debug.Log("심교수님 이벤트1 : " + PlayerPrefs.GetInt("shim"));    //결과 확인하기
            SceneManager.LoadScene("Shim_Event2");
        }
        if (theChoice.GetResult() == 2) // 자전거
        {
            PlayerPrefs.SetInt("shim", PlayerPrefs.GetInt("shim") + 10);
            Debug.Log("심교수님 이벤트1 : " + PlayerPrefs.GetInt("shim"));    //결과 확인하기
            SceneManager.LoadScene("Shim_Event2");
        }
        if (theChoice.GetResult() == 3) // 걷기
        {
            PlayerPrefs.SetInt("shim", PlayerPrefs.GetInt("shim") + 5);
            Debug.Log("심교수님 이벤트1 : " + PlayerPrefs.GetInt("shim"));    //결과 확인하기
            SceneManager.LoadScene("Shim_Event2");
        }
